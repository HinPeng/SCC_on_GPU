...
__host__
void TRIMMING(unsigned subnum, vector<unsigned> &V, vector<unsigned> &E);
__host__
void PIVOTS_SEL(unsigned subnum,unsigned &pivot_h,unsigned &termin);
__host__
void Updata(unsigned cur_max_sub,unsigned subnum,vector<unsigned> &visF,vector<unsigned> &visB,vector<unsigned> &scc,unsigned &termin);
/*__host__
...
int main()
{
	...
	while(flag){
		...
		for(unsigned i = st; i <= ed; i++){
			...
			Updata(cur_max_sub,i,visF,visB,scc,termin);
			cout<<termin<<endl;
...
void BFS(vector<unsigned> &V, vector<unsigned> &E, unsigned sourceVertex,vector<unsigned> &visited)
{
	...
	while (true) {
		...
		// launch kernel 1
		BFSKernel1 <<<gridSizeK1, BLOCK_SIZE >>> (N, activeMask, d_V, d_E, d_F, d_X);
...
void TRIMMING(unsigned subnum,vector<unsigned> &V, vector<unsigned> &E)
{
	...
	while (true) {
		...
		// launch kernel 2
		TRIMKernel << <gridSizeK2, BLOCK_SIZE >> > (N, subnum,d_V, d_E);
		//gpuErrchk(cudaPeekAtLastError());
		gpuErrchk(cudaDeviceSynchronize());

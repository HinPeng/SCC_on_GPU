...
int main()
{
	vector<unsigned> VF, EF, VB, EB, visF, visB,prefixsum_;
	...
	gpuErrchk(cudaMalloc(&d_p, Bou*sizeof(unsigned)));
	gpuErrchk(cudaMemset(d_p, 255, Bou*sizeof(unsigned)));
	gpuErrchk(cudaMemcpyToSymbol(pivot, &d_p, sizeof(unsigned *),size_t(0), cudaMemcpyHostToDevice));

	gpuErrchk(cudaMalloc(&d_prefixsum, (Bou+1)*sizeof(unsigned)));
	...
	while(true){
		...
		if((9*index_e+8) >= Bou){
			...
			if(termin == FALSE){
				gpuErrchk(cudaMemcpy(d_prefixsum, prefixsum_.data(), (Bou+1)*sizeof(unsigned), cudaMemcpyHostToDevice));
				RenumberKernel<<<gridSizeK2,BLOCK_SIZE>>>(N,d_prefixsum);
				gpuErrchk(cudaDeviceSynchronize());
				index_e = prefixsum_.at(Bou) - 1;
			}
		}
		else{
...
void BFS(vector<unsigned> &V, vector<unsigned> &E, unsigned index_s, unsigned index_e, vector<unsigned> &visited)
{
	...
	const size_t gridSizeK3 =
			(Bou + BLOCK_SIZE - 1) / BLOCK_SIZE;

	ReadPivot<<<gridSizeK3,BLOCK_SIZE>>>(Bou,d_p);
	gpuErrchk(cudaDeviceSynchronize());
	cudaMemcpy(pivot_h.data(), d_p, Bou*sizeof(unsigned), cudaMemcpyDeviceToHost);
...
{
	prefixsum.clear();
	prefixsum.resize(Bou+1);

	unsigned *d_vf,*d_vb,*d_ac;
	unsigned *prefixSums;

	size_t memSize = (N + 1) * sizeof(unsigned);
	size_t preSize = (Bou + 1)*sizeof(unsigned);

	gpuErrchk(cudaMalloc(&d_ac, preSize - sizeof(unsigned)));
	gpuErrchk(cudaMemset(d_ac, FALSE, preSize - sizeof(unsigned)));
	...
	gpuErrchk(cudaMalloc(&prefixSums, preSize));
	preallocBlockSums(Bou + 1);
	...
	if(termin == FALSE){
		prescanArray(prefixSums, d_ac, Bou + 1);
		//gpuErrchk(cudaMemcpyToSymbol(prefixsum, &prefixSums, sizeof(unsigned *),size_t(0), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(prefixsum.data(), prefixSums, preSize, cudaMemcpyDeviceToHost));
	}
